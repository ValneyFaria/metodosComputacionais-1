...
void main() {
	...
	printf("Este e' o metodo iterativo de Gauss-Seidel.\n");
	...
	do {
		...
		maiorx = 0;
		maiorb = 0;
		//Calcula os proximos 'x' e armazena em 'b' os 'x' antigos para usar na subtracao
		for(i = 0; i < n; i++) {
			b[i] = x[i];
			for(j = 0; j < n; j++) {
				soma = soma + c[i][j] * x[j];
			...
			if((fabs(x[i]) - fabs(b[i])) > maiorb) {
				maiorb = (fabs(x[i]) - fabs(b[i]));
			}
		}
